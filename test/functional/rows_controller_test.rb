...
class RowsControllerTest < ActionController::TestCase
  ...
  test "should create row" do
    assert_difference('Row.count') do
      post :create, row: { additional_styles: @row.additional_styles, additional_tags: @row.additional_tags, page_id: @row.page_id, position: @row.position }
    end
  ...
  test "should update row" do
    put :update, id: @row, row: { additional_styles: @row.additional_styles, additional_tags: @row.additional_tags, page_id: @row.page_id, position: @row.position }
    assert_redirected_to row_path(assigns(:row))
  end
